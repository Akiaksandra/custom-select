...
document.addEventListener("DOMContentLoaded", () => {
  ...
  input.addEventListener("input", (e) => {
    ...
    createDataList(inputValue);
  });

  input.addEventListener("focus", (e) => {
    selectList.classList.remove("hide");
    inputArrow.classList.add("open");
  });
  ...
  const createDataList = (inputValue) => {
    selectList.innerHTML = "";
    if (inputValue) {
      const filterData = data.filter((el) => el.title.includes(inputValue));
      if (filterData.length === 0) {
        const newItem = createListItem("Ничего не найдено");
        selectList.append(newItem);
        return;
      }
      filterData.forEach((el) => {
        const newItem = createListItem(el.title);
        selectList.append(newItem);
      });
    } else {
      data.forEach((el) => {
